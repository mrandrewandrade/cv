...
with open('professional.txt') as fp:
    content  = fp.readlines()

all_ind_exp = []
print type(content)
for iii in range (0,len(content)/3):
    ind_exp = dict()
    ind_exp["position"]=content[iii*3]
    ind_exp["company"] = content[iii*3+1]
    ind_exp["date"] = content[iii*3+2] + ", "
    ind_exp["caption"] = "insert caption"
    ind_exp["thumbnail"] = "/img/" + content [iii*3+1] + ".jpg"
